...
- **Module Versioning** — Automatic version snapshots on every import (AI Designer, BPMN) and manual version creation. Compare any two versions with a structured side-by-side diff showing line-by-line changes for scripts, forms, templates, routes, and more. Rollback to any previous state with dependency validation warnings. Add comments to track changes over time.
...
- **AI Enhancement for Individual Elements** — Get targeted AI assistance on scripts, forms, templates, and routes without building entire modules. Features include: Script Enhancement (improve error handling, documentation, structure), Form Schema Generation (describe forms in natural language), Template Improvement (fix HTML, add accessibility/responsiveness), and Route Configuration Helper (suggest route settings from descriptions). All features include diff previews and one-click apply.
