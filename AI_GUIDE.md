...
27. **Use actual table names in SQL, not model class names** — In `<sql>` queries, platform tables use snake_case names, not the Python model class names. Key mappings:
    ...
    DynamicModel tables use the name passed to `get_or_create()` lowercased (e.g. `SalesRecord` → `salesrecord`).
    DynamicModel tables are automatically registered in the `dynamic_table_registry` table with the module that created them. This powers the "Module" column in the Data browser and enables "Drop tables" on module delete. No manual registration needed — the script runner injects the current module's ID automatically when scripts call `DynamicModel.get_or_create()`.
28. **Incoming emails are in `incoming_emails` table** — When the IMAP polling feature is enabled, incoming emails are stored in the `incoming_emails` table. Modules can query this table to process emails. Key columns: `id`, `message_id`, `subject`, `from_address`, `to_address`, `body_text`, `body_html`, `attachments` (comma-separated filenames), `module_slug` (for module claiming), `processed` (boolean), `created_at`, `processed_at`. Use `MODULE_SLUG` (which resolves to your module's slug) to claim emails: `UPDATE incoming_emails SET module_slug = 'MY_SLUG', processed = 1, processed_at = datetime('now') WHERE processed = 0 AND subject LIKE '%support%' LIMIT 1`. Modules can also delete emails directly with `DELETE FROM incoming_emails WHERE id = X` if cleanup is needed rather than just marking processed. Note: the IMAP settings (host, port, username, password, folder, poll interval) are configured in the platform Settings page, not at the module level.
29. **Use `get_credential()` for secrets** — API keys, tokens, and passwords stored via the Credentials admin page are encrypted at rest. Scripts access them with `get_credential('name')`, which returns the decrypted value. Credentials are module-scoped: a script can only access credentials belonging to its own module. Available credential types: `api_key`, `oauth_token`, `basic_auth`, `custom`. Store them in the admin panel (`/__admin/credentials`) — never hardcode secrets in script source code.
30. **Use `call_api()` for HTTP requests** — The built-in `call_api()` function provides retry logic, timeout, and consistent error handling without importing `urllib` directly. Signature: `call_api(method='GET', url, headers=None, json=None, data=None, timeout=30, retries=3, backoff=2)`. Returns a dict with `status_code`, `headers`, `body` (parsed JSON dict or text), `elapsed_ms`, and optional `error` on failure. Example: `result = call_api('GET', 'https://api.github.com/repos/user/repo', headers={'Authorization': 'Bearer ' + get_credential('github_token')})`.
31. **Dynamic tables are auto-registered** — When your script calls `DynamicModel.get_or_create()`, the table is automatically registered in `dynamic_table_registry` with your module as the owner. This persists across restarts and survives script edits. The Data browser shows the owning module, and the delete module dialog lets you drop owned tables. The module ID is injected automatically by the script runner — you don't need to pass it manually.
32. **Declare Python dependencies in module XML** — If your module needs extra libraries (e.g., `requests`, `pandas`, `pillow`), add a `<requirements>` element in the XML bundle. Each line is passed to `pip install` on import. Example:
    ```xml
    <requirements>
    requests==2.31.0
    pandas
