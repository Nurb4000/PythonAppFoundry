...
def import_module(xml_str, update_existing=False, module_id=None):
    ...
    # Requirements are recorded but NOT auto-installed.
    # Installing packages during import runs arbitrary code from untrusted XML,
    # so we surface them for explicit admin approval instead (see requirements_text).
