...
def list_scripts():
    return list_view(Script, 'scripts', ['id', 'name', 'language'], 'admin.edit_script', 'admin.new_script', has_module=True)
...
def new_script():
    ...
    return render_admin('New Script', NEW_SCRIPT_TEMPLATE, modules=modules)


NEW_SCRIPT_TEMPLATE = '''<form method="POST">
...
def edit_script(id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect(url_for('admin.list_scripts'))
    return render_admin('Edit Script', EDIT_SCRIPT_TEMPLATE, s=s, modules=modules)


EDIT_SCRIPT_TEMPLATE = '''<form method="POST">
  <input type="hidden" name="csrf_token" value="{{ csrf_token() }}">
  ...
  <label style="display:block;margin-top:12px;">Description<textarea name="description" style="width:100%;min-height:80px;resize:vertical;">{{ s.description }}</textarea></label>
  <label style="display:block;margin-top:12px;">Source Code<textarea name="source_code" id="source_code" rows="15" style="width:100%;font-family:monospace;z-index:1;background:rgba(255,255,255,0.9);">{{ s.source_code }}</textarea></label>
...
function testScript(scriptId) {
  ...
  fetch('/__admin/scripts/test/' + scriptId, { method: 'POST', headers: {'X-CSRFToken': document.querySelector('[name=csrf_token]').value}, })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (data.error) { result.innerHTML = '<div style="background:#fee;border:1px solid #fcc;padding:1rem;border-radius:4px;"><strong style="color:#c00;">Error:</strong><pre style="margin:0.5rem 0;font-size:0.85em;overflow:auto;">' + data.error + '</pre></div>' + (data.output ? '<div style="background:#f4f4f4;border:1px solid #ddd;padding:1rem;border-radius:4px;margin-top:0.5rem;"><strong>Output:</strong><pre style="margin:0.5rem 0;font-size:0.85em;overflow:auto;">' + data.output + '</pre></div>' : ''); }
      else { result.innerHTML = '<div style="background:#efe;border:1px solid #cfc;padding:1rem;border-radius:4px;"><strong style="color:#080;">Success</strong> (' + data.duration_ms + 'ms)</div>' + (data.result ? '<div style="background:#f4f4f4;border:1px solid #ddd;padding:1rem;border-radius:4px;margin-top:0.5rem;"><strong>Result:</strong><pre style="margin:0.5rem 0;font-size:0.85em;overflow:auto;">' + data.result + '</pre></div>' : '') + (data.output ? '<div style="background:#f4f4f4;border:1px solid #ddd;padding:1rem;border-radius:4px;margin-top:0.5rem;"><strong>Output:</strong><pre style="margin:0.5rem 0;font-size:0.85em;overflow:auto;">' + data.output + '</pre></div>' : ''); }
    })
    .catch(function(err) { result.innerHTML = '<div style="background:#fee;border:1px solid #fcc;padding:1rem;border-radius:4px;"><strong style="color:#c00;">Request failed:</strong> ' + err.message + '</div>'; });
}
</script>'''
...
def debug_script(id):
    ...
    return render_admin('Debug: ' + s.name, DEBUG_SCRIPT_TEMPLATE, s=s, duration=duration, source_lines=source_lines, error=error, output=output)


DEBUG_SCRIPT_TEMPLATE = '''<h2>Debug: {{ s.name }}</h2>
...
{% if error %}<h3 style="color:#c00;">Error</h3><pre style="background:#fff5f5;padding:0.5rem;overflow:auto;font-size:0.85rem;border:1px solid #fcc;border-radius:4px;">{{ error }}</pre>{% elif output %}<h3>Output</h3><pre style="background:#f4f4f4;padding:0.5rem;overflow:auto;font-size:0.85rem;border:1px solid #ddd;border-radius:4px;">{{ output }}</pre>{% else %}<p>Script completed with no output.</p>{% endif %}
