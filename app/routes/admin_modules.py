...
from flask import Blueprint, request, redirect, url_for, render_template_string, flash, abort
...
def list_modules():
    ...
    content = render_template_string('''<div style="display:flex;gap:0.75rem;align-items:center;margin-bottom:1rem;">
...
@modules_bp.route('/modules/edit/<int:id>', methods=['GET', 'POST'])
@developer_or_admin_required
def edit_module(id):
...
@modules_bp.route('/modules/delete/<int:id>', methods=['GET', 'POST'])
@developer_or_admin_required
@csrf_protect
...
''', name=name, dependencies=dependencies)
    ...
    # Explicitly delete related objects
    for route in m.routes.all():
        db.session.delete(route)
    for script in m.scripts.all():
        db.session.delete(script)
    for form in m.forms.all():
        db.session.delete(form)
    for task in m.scheduled_tasks.all():
        db.session.delete(task)
    for trigger in m.triggers.all():
        db.session.delete(trigger)
    for credential in m.credentials.all():
        db.session.delete(credential)
    for version in m.versions.all():
        db.session.delete(version)
    for dep in m.dependencies_from.all():
        db.session.delete(dep)
    for dep in m.dependencies_to.all():
        db.session.delete(dep)
...
@modules_bp.route('/modules/<int:id>/reset', methods=['POST'])
@admin_required
@csrf_protect
def reset_system_module(id):
    ...
    for trigger in m.triggers.all():
        db.session.delete(trigger)
    for query in m.query_reports.all():
...
@modules_bp.route('/modules/<int:module_id>/scan-dependencies', methods=['POST'])
@developer_or_admin_required
@csrf_protect
...
@modules_bp.route('/modules/<int:module_id>/dependencies')
@developer_or_admin_required
...
@modules_bp.route('/modules/clone/<int:id>', methods=['POST'])
@developer_or_admin_required
@csrf_protect
def clone_module(id):
    m = Module.query.get_or_404(id)
...
@modules_bp.route('/modules/import_xml/<int:id>', methods=['POST'])
@developer_or_admin_required
@csrf_protect
