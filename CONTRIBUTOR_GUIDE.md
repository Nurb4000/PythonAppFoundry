...
| `app/services/bundle.py` | ~400 | Module XML serialization/deserialization. `export_module()` walks model relationships and builds XML (including credentials, settings, requirements). `import_module()` parses XML, creates/updates module and all children, creates placeholder credentials, sets default settings, bumps version, detects dependencies, installs requirements. |
...
| `app/services/versioning.py` | ~200 | Module version snapshots — exports module to XML, stores as `ModuleVersion` with auto-incremented patch version. `create_version()` / `restore_version()` for snapshot management. `structured_diff_versions()` parses both XML snapshots and compares element-by-element (scripts, routes, forms, templates, tasks, triggers, query reports, credentials) producing categorized added/removed/modified lists with line-by-line diffs. `validate_restore_dependencies()` scans snapshot XML for cross-module references before rollback. |
